...
PcombDetection = {
    ...
    analyzeBodySupport = function(body, impactPoint)
        ...
        -- NEW: Check if COM is within support footprint; if not, prefer collapse
        local comSupported = PcombDetection.isCOMSupported(body, supportAnalysis, comWorldPos)
        if not comSupported then
            if GetBool("savegame.mod.pcomb.global.debug") then
                DebugPrint("COM not supported for body " .. tostring(body) .. ", forcing collapse check")
            end
        end
    ...
    -- 2D point-in-polygon test (ray-casting) for X/Z plane
    isPointInPolygon2D = function(polygon, point)
        if not polygon or #polygon < 3 then return false end
        local x, z = point[1], point[2]
        local inside = false
        for i = 1, #polygon do
            local j = (i % #polygon) + 1
            local xi, zi = polygon[i][1], polygon[i][2]
            local xj, zj = polygon[j][1], polygon[j][2]
    ...
    -- Determine if the center of mass projects into the support footprint
    isCOMSupported = function(body, supportAnalysis, comWorldPos)
        -- If no support area, it's not supported
        if not supportAnalysis or supportAnalysis.totalSupportArea <= 0 or #supportAnalysis.supportingBodies == 0 then
            return false
        end

        -- Build a simple support polygon from supporting bodies' approximate contact centroids (X,Z)
        local polygon = {}
        local bodyBounds = GetBodyBounds(body)
        if not bodyBounds then return false end

        for _, supportBody in ipairs(supportAnalysis.supportingBodies) do
            local sb = GetBodyBounds(supportBody)
            if sb and type(sb) == "table" and #sb >= 6 then
                -- Use the overlap centroid between this body and the support as a contact point
                local overlapX1 = math.max(bodyBounds[1], sb[1])
                local overlapX2 = math.min(bodyBounds[4], sb[4])
                local overlapZ1 = math.max(bodyBounds[3], sb[3])
                local overlapZ2 = math.min(bodyBounds[6], sb[6])

                if overlapX2 > overlapX1 and overlapZ2 > overlapZ1 then
                    local cx = (overlapX1 + overlapX2) / 2
                    local cz = (overlapZ1 + overlapZ2) / 2
                    table.insert(polygon, {cx, cz})
                else
                    -- Fallback to support body's top-center
                    local cx = (sb[1] + sb[4]) / 2
                    local cz = (sb[3] + sb[6]) / 2
                    table.insert(polygon, {cx, cz})
                end
            end
        end

        -- If polygon is degenerate build a simple rectangle from totalSupportArea center
        if #polygon < 3 then
            -- create small footprint around average support center
            local avgX, avgZ = 0, 0
            for i = 1, #polygon do avgX = avgX + polygon[i][1]; avgZ = avgZ + polygon[i][2] end
            if #polygon > 0 then avgX = avgX / #polygon; avgZ = avgZ / #polygon else
                -- fallback: project COM to ground - assume unsupported
                return false
            end
            local eps = 0.01
            polygon = {
        ...
        -- Project COM to X,Z plane and test
        local com2D = {comWorldPos[1], comWorldPos[3]}

        -- Respect user-configurable margin to make the COM support test tunable.
        local margin = GetFloat("savegame.mod.pcomb.ibsit.com_margin") or 0.0
        if margin and margin > 0 and #polygon >= 3 then
            ...
            -- Scale polygon vertices outwards from centroid by (1 + margin)
            local scale = 1 + margin
            local scaled = {}
            for i = 1, #polygon do
                local vx = polygon[i][1] - cx
                local vz = polygon[i][2] - cz
                table.insert(scaled, {cx + vx * scale, cz + vz * scale})
            end

            if PcombDetection.isPointInPolygon2D(scaled, com2D) then
                return true
            end
            -- fallthrough to strict polygon test if scaled doesn't contain point
        end

        return PcombDetection.isPointInPolygon2D(polygon, com2D)
    end,
...
PcombEffects = {
    ...
    applyImpactDamage = function(body, fallData, impactVelocity)
        ...
        -- Validate fallData and required fields before doing arithmetic
        if not fallData or not fallData.effectiveMass or not fallData.maxVelocity then
        ...
        local impactPosition = bt.pos

        -- Coerce numeric inputs to avoid nil/invalid values
        local effectiveMass = tonumber(fallData.effectiveMass) or 0
        local maxVel = tonumber(fallData.maxVelocity) or 0
        local impactForce = effectiveMass * maxVel

        -- Apply configuration multiplier (ensure numeric)
        local configMultiplier = tonumber(GetFloat("savegame.mod.pcomb.impact.multiplier")) or 1.0
        if type(impactForce) ~= "number" then impactForce = 0 end
        impactForce = impactForce * configMultiplier

        -- If there's no meaningful impact force, skip further processing
        if impactForce <= 0 then
            return
        end
        ...
        local matName = ""
        if fallData.materialProps and fallData.materialProps.name then
            matName = fallData.materialProps.name
        end

        if matName == "Stone" or matName == "Concrete" then
        ...
        local impactRadius = math.max((finalDamage or 0) * 0.5, configRadius) -- Damage radius based on impact force
        if not impactRadius or impactRadius <= 0 then
            impactRadius = configRadius or 0.01
        end
...
function tick(dt)
    ...
    -- Handle tab navigation in pause menu
    if pauseMenuEnabled then
        if InputPressed("control") and InputDown("tab") then
            -- Shift+Tab to go to previous tab
            pauseMenuCurrentTab = pauseMenuCurrentTab - 1
            if pauseMenuCurrentTab < 1 then
                pauseMenuCurrentTab = #PcombSettings.tabs
            end
        elseif InputPressed("tab") then
            -- Tab to go to next tab
            pauseMenuCurrentTab = pauseMenuCurrentTab + 1
            if pauseMenuCurrentTab > #PcombSettings.tabs then
                pauseMenuCurrentTab = 1
            end
        end
    end
    
    -- Close menu with Escape key
    if pauseMenuEnabled and InputPressed("escape") then
        pauseMenuEnabled = false
    end
    
    -- Only process physics if menu is not enabled (this effectively "pauses" physics processing)
...
function draw(dt)
    ...
    if pauseMenuEnabled then
        -- Enable mouse capture for the menu
        UiMakeInteractive()
        
        -- Calculate menu position and size
        local menuX = UiCenter() - 300
        local menuY = UiMiddle() - 250
        local menuWidth = 600
        local menuHeight = 500
        
        -- Background
        UiPush()
        UiTranslate(menuX, menuY)
        UiColor(0, 0, 0)
        UiImageBox("ui/common/box-solid-6.png", menuWidth, menuHeight, 6, 6)
        UiPop()
        
        -- Title
        UiPush()
        UiTranslate(menuX + 20, menuY + 20)
        UiAlign("left top")
        UiFont("bold.ttf", 24)
        UiColor(1, 1, 1)
        ...
        -- Tab navigation
        UiPush()
        UiTranslate(menuX + 20, menuY + 60)
        UiAlign("left top")
        UiFont("regular.ttf", 14)
        
        for i, tabName in ipairs(PcombSettings.tabs) do
            UiPush()
            if pauseMenuCurrentTab == i then
                UiColor(0.3, 0.6, 1.0)
            else
                UiColor(0.4, 0.4, 0.4)
            end
            
            if UiTextButton(tabName) then
                pauseMenuCurrentTab = i
            end
            UiPop()
            UiTranslate(110, 0)
        end
        ...
        -- Content area
        local contentY = menuY + 100
        local contentX = menuX + 20
        
        UiPush()
        UiTranslate(contentX, contentY)
        
        -- Render selected tab content using centralized functions
        if pauseMenuCurrentTab == 1 then
            PcombSettings.drawGlobalTab()
        elseif pauseMenuCurrentTab == 2 then
            PcombSettings.drawPRGDCoreTab()
        elseif pauseMenuCurrentTab == 3 then
            PcombSettings.drawPRGDAdvancedTab()
        elseif pauseMenuCurrentTab == 4 then
            PcombSettings.drawIBSITCoreTab()
        elseif pauseMenuCurrentTab == 5 then
            PcombSettings.drawIBSITAdvancedTab()
        elseif pauseMenuCurrentTab == 6 then
            PcombSettings.drawMBCSTab()
        elseif pauseMenuCurrentTab == 7 then
            PcombSettings.drawPerformanceTab()
        elseif pauseMenuCurrentTab == 8 then
            PcombSettings.drawMaterialsTab()
        end
        
        UiPop()
        
        -- Performance info and close button
        UiPush()
        UiTranslate(menuX + 20, menuY + menuHeight - 50)
        UiAlign("left top")
        UiFont("regular.ttf", 14)
        UiColor(0.8, 0.8, 0.8)
        UiText("FPS: " .. sharedState.performance.fps .. " | Bodies: " .. sharedState.performance.bodiesProcessed)
        UiPop()
        
        -- Close button
        UiPush()
        UiTranslate(menuX + menuWidth - 80, menuY + menuHeight - 40)
        UiAlign("left top")
